...
(function() {
  ...
  webcam.startStream = function() {
    ...
    trace(
      "webcam.startStream: try: " +
        webcam.constraints.video.width +
        "x" +
        webcam.constraints.video.height
    );

    window.navigator.mediaDevices
      .getUserMedia(webcam.constraints)
  ...
  window.cameraStop = webcam.stopStream;
  ...
  webcam.onStreamDimensionsAvailable = function() {
    ...
    if (webcam.video.videoWidth === 0) {
      ...
      webcam.onStreamDimensionsAvailable_timeout = setTimeout(
        webcam.onStreamDimensionsAvailable,
        100
      );
    } else {
  ...
  webcam.setup = function(
    ...
    if (!webcamVideo || !imageDataCanvas) {
      trace(
        "setupWebcam: Please add a <video> tag with id='_webcam' and " +
          "a <canvas> tag with id='_imageData' to the DOM.",
        true
      );
      return;
    }
